...
##2.Random Walk Forest
```{r}
rwf_forecast<- rwf(t1,h=4)
rwf_forecast
plot(rwf_forecast)
```
# checking accuracy for rwf
```{r}
...
model
summary(model)
plot(Final$air_temp,Final$footfalls,pch=19, col="blue", main="Scatter Plot")
```
#correlation check
```{r}
...
#Plotting R2 line and residual vs fitted
```{r}
